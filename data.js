...
async function tmp(){
    ...
    let result = Array.from({ length: count_answers.length }, (_, i) => true_false.slice(cumulative_counts[i], cumulative_counts[i + 1]));
    answers = Array.from({ length: count_answers.length }, (_, i) => answers.slice(cumulative_counts[i], cumulative_counts[i + 1]));
    ...
    for (let i = 0; i < questions.length; i++) {
        ...
        res.push([questions[i].slice(questions[i].indexOf('.') + 1).trim()].concat(answers[i]));
    }
...
async function main(res) {
    ...
    files = Object.keys(files).sort((a, b) => parseInt(a.match(/\d+/)[0]) - parseInt(b.match(/\d+/)[0]));
